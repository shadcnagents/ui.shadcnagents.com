...
// ---------------------------------------------------------------------------
// Theme presets — overrides both base tokens AND Tailwind v4 --color-* vars
// so utilities like bg-primary actually pick up the change.
// ---------------------------------------------------------------------------
function makeThemeVars(base: Record<string, string>) {
  const vars: Record<string, string> = {}
  for (const [key, value] of Object.entries(base)) {
    vars[key] = value
    // Also set the Tailwind v4 --color-* mapping
...
export default function StackPage({ params }: StackPageProps) {
  ...
  const [activeRadius, setActiveRadius] = useState(2) // 0.5rem default
  ...
  const stackIndex = allStacks.findIndex(
    (p) => p.link === `/stacks/${slug}`
  )
  ...
  const nextStack =
    stackIndex < allStacks.length - 1
      ? allStacks[stackIndex + 1]
      : null
  ...
  // Compute inline styles for preview container
          ...
          <div className="flex items-center gap-0.5 rounded-md border border-border p-0.5">
            ...
            <button
              onClick={() => {
                ...
                params.set("radius", radiusOptions[activeRadius].value || "0")
                if (customColor) params.set("color", customColor)
                window.open(`/preview/${slug}?${params.toString()}`, "_blank")
              }}
              ...
              {isPro && source && (
                <div className="absolute inset-0 top-[140px]">
                  ...
                  <div className="flex h-full flex-col items-center bg-background/80 pt-8 backdrop-blur-sm">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground/50">
                      Unlock with Pro for full source code
                    </p>
                    <Button asChild size="sm" className="mt-4">
                      <Link href="/pricing">Unlock with Pro</Link>
                    </Button>
