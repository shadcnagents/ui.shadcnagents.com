...
export function repairJSON(input: string): { output: string; repairs: RepairAction[] } {
...
export function validateSchema(
  ...
  for (const field of schema) {
    ...
    if (value !== undefined && value !== null) {
      ...
      const expectedTypes = field.type.toLowerCase().split("|").map((t) => t.trim())
...
export function useStructuredOutput(options: UseStructuredOutputOptions = {}) {
  ...
  const validateOutput = useCallback(
      ...
      try {
        while (attempts < maxRetries) {
          ...
          if (parseError && enableAutoRepair) {
            const { output: repairedOutput, repairs } = repairJSON(currentOutput)
          ...
          if (parseError && enableCorrectivePrompting && retryFn && attempts < maxRetries) {
          ...
          if (!parseError && parsed !== undefined) {
            const schemaErrors = schema.length > 0 ? validateSchema(parsed, schema) : []
    ...
    [maxRetries, enableAutoRepair, enableCorrectivePrompting, strictMode, schema, onValidationSuccess, onValidationFailure, onRepair]
  )

  // Quick parse without retries
  const quickParse = useCallback(
    (rawOutput: string): { success: boolean; data: unknown; error?: string } => {
      try {
        const { output } = enableAutoRepair ? repairJSON(rawOutput) : { output: rawOutput }
...
export function ValidationStatusBadge({ result, size = "md" }: ValidationStatusBadgeProps) {
  ...
  return (
    ...
    >
      ...
      <span className="opacity-60">{result.attempts} attempt{result.attempts > 1 ? "s" : ""}</span>
...
export function RepairLog({ repairs }: RepairLogProps) {
  ...
  const repairTypeConfig: Record<RepairType, { icon: React.ReactNode; color: string }> = {
    fix_truncation: { icon: <ScissorsIcon className="size-3.5" />, color: "text-red-500" },
    fix_trailing_comma: { icon: <CommaIcon className="size-3.5" />, color: "text-amber-500" },
    fix_single_quotes: { icon: <QuoteIcon className="size-3.5" />, color: "text-blue-500" },
    fix_unquoted_keys: { icon: <KeyIcon className="size-3.5" />, color: "text-purple-500" },
    fix_missing_brackets: { icon: <BracketsIcon className="size-3.5" />, color: "text-orange-500" },
    fix_code_block: { icon: <CodeIcon className="size-3.5" />, color: "text-cyan-500" },
    fix_extra_content: { icon: <TrashIcon className="size-3.5" />, color: "text-pink-500" },
    strip_markdown: { icon: <FileTextIcon className="size-3.5" />, color: "text-emerald-500" },
  }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex gap-2 text-[10px] font-mono">
                      <div className="flex-1 rounded bg-red-500/10 px-2 py-1 text-red-600 dark:text-red-400 truncate">
                        <span className="opacity-50">-</span> {repair.before || "(empty)"}
                      </div>
                      ...
                      <div className="flex-1 rounded bg-emerald-500/10 px-2 py-1 text-emerald-600 dark:text-emerald-400 truncate">
                        <span className="opacity-50">+</span> {repair.after || "(empty)"}
                      </div>
                    </div>
                  </div>
                </div>
  ...
  if (errors.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 text-center">
        ...
        <div className="text-sm text-muted-foreground">All validations passed</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-red-600 dark:text-red-400 mb-3">
        ...
        <span className="text-xs font-medium">{errors.length} validation error{errors.length > 1 ? "s" : ""}</span>
      </div>
...
export function ValidatorStatsPanel({ stats }: ValidatorStatsPanelProps) {
  const successRate = stats.totalValidations > 0
    ...
    : 0
  const repairRate = stats.totalValidations > 0
        ...
        <div className="mt-4">
          ...
          <div className="flex h-2 overflow-hidden rounded-full bg-muted">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${((stats.successCount - stats.repairCount) / Math.max(1, stats.totalValidations)) * 100}%` }}
              transition={{ duration: 0.5 }}
            />
            <motion.div
              className="bg-amber-500"
              initial={{ width: 0 }}
              animate={{ width: `${(stats.repairCount / Math.max(1, stats.totalValidations)) * 100}%` }}
              transition={{ duration: 0.5 }}
            />
            <motion.div
              className="bg-red-500"
              initial={{ width: 0 }}
              animate={{ width: `${(stats.failureCount / Math.max(1, stats.totalValidations)) * 100}%` }}
              transition={{ duration: 0.5 }}
            />
          </div>
        </div>
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Repaired</span>
        </div>
        <pre className="text-xs font-mono whitespace-pre-wrap break-all text-muted-foreground max-h-[200px] overflow-auto">
          {formatJSON(repaired)}
        </pre>
